...
use quantization::pq::pq::{ProductQuantizerConfig, CODEBOOK_NAME, CONFIG_NAME};
    ...
    fn write_common_ivf_quantizer(ivf_directory: &str, config: &CollectionConfig) -> Result<()> {
        ...
        match config.quantization_type {
            QuantizerType::ProductQuantizer => {
                ...
                let config_path = Path::new(&ivf_quantizer_directory).join(CONFIG_NAME);
...
mod tests {
    ...
    use config::collection::CollectionConfig;
    use quantization::pq::pq::CONFIG_NAME;
    use tempdir::TempDir;
    ...
    fn test_write_with_pq() {
        ...
        let ivf_quantizer_config_path = format!("{}/{}", ivf_quantizer_directory, CONFIG_NAME);
