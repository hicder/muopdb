...
impl MutableSegment {
    ...
    pub fn insert_for_user(
        ...
        if self.flushing_started.load(std::sync::atomic::Ordering::Acquire) {
            if let Some(invalidated_id_set) = self.invalidated_ids_per_user.write().get_mut(&user_id) {
    ...
    pub fn start_being_flushed(&mut self) {
        self.flushing_started.store(true, std::sync::atomic::Ordering::Release);
    }
...
mod tests {
    ...
    async fn test_mutable_segment() {
        ...
        let mut mutable_segment = MutableSegment::new(
