...
pub struct MutableSegment {
    ...
    invalidated_ids: RwLock<HashMap<u128, HashSet<u128>>>,
...
impl MutableSegment {
    pub fn new(config: CollectionConfig, base_directory: String) -> Result<Self> {
        Ok(Self {
            ...
            invalidated_ids: RwLock::new(HashMap::new()),
            finalized: false,
    ...
    pub fn insert(&self, doc_id: u128, data: &[f32]) -> Result<()> {
        ...
        if let Some(invalidated_id_set) = self.invalidated_ids.write().get_mut(&0) {
    ...
    pub fn invalidate(&self, user_id: u128, doc_id: u128) -> Result<bool> {
        if self.finalized {
            return Err(anyhow::anyhow!("Cannot invalidate doc_id from a finalized segment"));
        }

        let effectively_invalidated = self.multi_spann_builder.invalidate(user_id, doc_id)?;
        if effectively_invalidated {
            let mut invalidated_map = self.invalidated_ids.write();
            ...
            let _ = invalidated_set.insert(doc_id);
        }
...
mod tests {
    ...
    async fn test_mutable_segment() {
        ...
        let mutable_segment = MutableSegment::new(
        ...
        assert!(mutable_segment.invalidate(0, 0).expect("Failed to invalidate"));
        assert!(!mutable_segment.invalidate(0, 0).expect("Failed to invalidate"));
        assert!(mutable_segment.insert(0, &[5.0, 6.0, 7.0, 8.0]).is_ok());
        assert!(mutable_segment.invalidate(0, 0).expect("Failed to invalidate"));

        let invalidated_map = mutable_segment.invalidated_ids.read();
