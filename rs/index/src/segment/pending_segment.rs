    ...
    pub fn apply_pending_deletions(&self) -> Result<()> {
        ...
        match &*internal_index {
            Some(index) => {
                ...
                let is_empty = fs::read_dir(&invalidated_ids_directory)?.next().is_none();
                if !is_empty {
                    return Err(anyhow!("Invalidated ids directory is not empty"));
                }
                ...
                for (user_id, doc_ids) in self.temp_invalidated_ids.read().iter() {
                    for doc_id in doc_ids {
                        if !index.invalidate(*user_id, *doc_id)? {
                            return Err(anyhow!(
                                "{} is already invalidated for user {}, which should never happen",
                                doc_id,
                                user_id
                            ));
                        }
                ...
                // Remove temporary invalidated ids storage.
                fs::remove_dir_all(&self.temp_invalidated_ids_storage.read().base_directory)?;
                Ok(())
            }
